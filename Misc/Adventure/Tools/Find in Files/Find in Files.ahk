...
SearchHistory := GetHistory("SearchHistory", 0)
ReplaceHistory := GetHistory("ReplaceHistory", 0)
...
w := GuiWidth - 8 - 6 - 84 - 6
Gui Add, Tab3, hWndhTab x8 y8 w%w% h260, Search
...
Gui Add, ComboBox, hWndg_hCbxSearch vTextToFind x111 y130 w%cw%, %SearchHistory%
Gui Add, Text, x22 y165 w86 h23 +0x200, Replace With:
Gui Add, ComboBox, hWndg_hCbxReplace vReplaceWith x111 y165 w%cw%, %ReplaceHistory%

Gui Add, Text, hWndhSep2 x22 y195 w%sw% h2 +0x10

; Search options
Gui Add, CheckBox, vChkRecurse x22 y196 w160 h23, &Search subdirectories
Gui Add, CheckBox, vChkSingleMatch x22 y219 w160 h23 %g_SingleMatch%, &One match per file
Gui Add, CheckBox, vChkMatchCase gSetExclusivity x190 y196 w160 h23, &Case sensitive
Gui Add, CheckBox, vChkWholeWords gSetExclusivity x190 y219 w160 h23, &Match whole words
Gui Add, CheckBox, vChkRegEx gSetExclusivity x358 y196 w160 h23, &Regular Expression
Gui Add, CheckBox, vChkBackslashes gSetExclusivity x358 y219 w160 h23, Convert &backslashes
Gui Add, CheckBox, vChkNotContaining x526 y196 w160 h23, &Not containing the text
Gui Add, CheckBox, vChkHexSearch gSetExclusivity x526 y219 w160 h23, &Hexadecimal search
...
w := GuiWidth - 16
h := GuiHeight - 265 - 8 - sbH
Gui Add, ListView, hWndg_hLvResults vLV gLvHandler x8 y265 w%w% h%h% +LV0x14000 -Multi
...
If (ParamCount) {
    ...
    If (g_ParamString != "") {
        GuiControl Text, %g_hCbxSearch%, %g_ParamString%
    }
...
OnSize:
    ...
    AutoXYWH("w",  hTab)
    AutoXYWH("w*", g_hCbxTarget, g_hCbxFilter, g_hCbxSearch)
...
PerformSearch:
    ...
    GuiControlGet TextToFind,, %g_hCbxSearch%, Text
    GuiControlGet ReplaceWith,, %g_hCbxReplace%, Text

    AddToHistory(g_hCbxTarget, CbxTarget)
    AddToHistory(g_hCbxFilter, Filters)
    AddToHistory(g_hCbxSearch, TextToFind)
    AddToHistory(g_hCbxReplace, ReplaceWith)
    KillSelection()
...
msgbox % A_LoopFileFullPath "  Replace With:" ReplaceWith
                    While (!f.AtEoF) {
                        If (g_Stoped) {
                            Break 3
                        }
...
SaveSettings() {
    ...
    SaveHistory(g_hCbxSearch, "SearchHistory")
    SaveHistory(g_hCbxReplace, "ReplaceHistory")
}
...
KillSelection() {
    ...
    SendMessage 0xB1, 0, 0,, % "ahk_id " . GetChild(g_hCbxSearchg)
}
...
GetIniFileLocation(Filename) {
    ...
    If (!FileExist(FullPath)) {
        AppCfgFile := A_ScriptDir . "\" . Filename
...
CreateIniFile() {
    ...
    If (!FileExist(g_IniFile)) {
        Sections := "[Options]`n`n[Window]`n`n[TargetHistory]`n`n[FilterHistory]`n`n[SearchHistory]`n`n[ReplaceHistory]`n"

        FileAppend %Sections%, %g_IniFile%, UTF-16
        If (ErrorLevel) {
            FileCreateDir %g_AppData%
            g_IniFile := A_ScriptDir . "\Find in Files.ini"
