...
tray.Add("Control to Updates`tCtrl or Shift",(*) => a_now)
...
WinSpyGui() {
    ...
    lv := oGui.Add("ListView", "w360 r7 ReadOnly -NoSortHdr -Multi", ["Title", "x", "y", "Width", "Height"])
    ...
    oGui.Add("Edit","w360 r11 ReadOnly vCtrl_VisText")
    oGui.Add("Text","cblue vCtrl_HeaderAllText","All Text:")
    oGui.Add("Edit","w360 r12 ReadOnly vCtrl_AllText")

    oGui.Add("Text",'cblue',"Status Bar Text:")
    oGui.Add("Edit","w360 r7 ReadOnly vCtrl_SBText")
    oGui.Add("Text","cRed w300 vAdminCheck")
...
WinSpySize(GuiObj, MinMax, Width, Height) {
    ...
    list := "Title,Ctrl,SBText,VisText,AllText,Freeze,HeaderVisText,HeaderAllText"
    moveList := 'HeaderVisText,HeaderAllText,VisText,AllText'
    Loop Parse list, ","
    {
        if InStr(moveList, A_LoopField)
            oGui["Ctrl_" A_LoopField].Move((ctrlW / 2),,(ctrlW / 2 ))
        else
...
TryUpdate() {
    ...
    if IsProcessElevated(t4)
    {
        UpdateText('AdminCheck', 'Admin Process' )
        oGui['AdminCheck'].opt('cRed')
    }
    else
    {
        oGui['AdminCheck'].opt('cBlue')
        UpdateText('AdminCheck', 'Normal Process' )
    }
