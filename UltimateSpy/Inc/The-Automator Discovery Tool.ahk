...
CR := MyGui.AddPicture('h' CH.h ' w' CH.w  ,a_scriptdir '\res\' (A_IsAdmin ?  'CrosshairbgRed' :'Crosshairbg') '.png')
...
LV := MyGui.AddListView("x+m ys r7 w400",['icon','Interface',"Simplicity","link to Source"])
...
Report := Gui("+AlwaysOnTop -DPIScale +Owner" MyGui.hwnd,"Submit Report")
...
Crosshair := Gui("-SysMenu +AlwaysOnTop -Border -Caption -DPIScale +Owner" MyGui.hwnd, 'Crosshair' )
Crosshair.AddPicture('x0 y0 h' CH.h ' w' CH.w   ,a_scriptdir '\res\' (A_IsAdmin ?  'CrosshairRed' :'Crosshair') '.png')
Crosshair.MarginX := 0
Crosshair.MarginY := 0
Crosshair.Show('hide')
...
{
    ; todo verify header 
    if LV.GetText(0,4) != "link to Source"
    {
        msgbox 'Columns has been changed'
        return
    }
    if row 
        run 'HTTP://' LV.GetText(row,4)
}
...
cols := Map(
		'Win32 Menu', {ico:"⚪️",status:isMenuAvailable(iHwnd),       score: 10,Link:'the-Automator.com/MenuWriter' },
		'Win32',      {ico:'⚫️',status:getWin32Controlcount(iHwnd),  score: 9,Link:'the-Automator.com/Controls'},
		'StatusBar',  {ico:'⚡️',status:instr(controlnn,'statusbar'), score: 9,Link:'the-Automator.com/StatusBar'},
		'ImageSearch',{ico:'⬜️',status:1,                            score: 8,Link:'the-Automator.com/FindText'},
		'COM',        {ico:'🔆',status:isCOMAvailable(iHwnd),        score: 7,link:'the-Automator.com/COM'},
		'Rufaydium',  {ico:'🔷',status:isBrowser(pid),               score: 6,Link:'the-Automator.com/Rufaydium'},
		'ACC',        {ico:'🅰',status:isACCAvailable(iHwnd),        score: 5,Link:'the-Automator.com/ACC'},
		'UIA',        {ico:'💊',status:isUIAAvailable(iHwnd),        score: 2,Link:'the-Automator.com/UIA'},
		'Messages',   {ico:'🔍',status:1,                            score: 1,Link:'the-Automator.com/messages'}
	)
    for title, info in cols
        if info.status
        {
            LV.Add(,info.ico,title,info.score,info.Link)
            Report.Data .= title ", "
        }
    ...
    LV.ModifyCol(3, 'Integer SortDesc Center')
...
{
    SB.SetText("checking for Win32 Controls")
    Win32 := 0
...
Ctrl_Click(*)
{
    ...
    ;isadmin.value := IsProcessElevated(ProcessExist(ProcessName))
...
SubmitUserReport(*)
{
    Report.submit()
    MyGui.Opt("+Disabled")
    ;info := Report['User'].value "`n"  Report['Email'].value "`n`n" Report.Data
    ;msgbox  info,, 'Owner' MyGui.hwnd
   
    ;email := TextEncode(Report['Email'].value)
